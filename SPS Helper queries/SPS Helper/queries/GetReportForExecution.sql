...
IF ( NOT EXISTS (
    SELECT TOP 1 1
        FROM
            ...
            INNER JOIN [ReportingService_test2DBTempDB].dbo.ExecutionCache AS EC ON C.ItemID = EC.ReportID
        WHERE
...
BEGIN   -- use cache
    ...
    FROM
        ExtendedCatalog(@OwnerID, @Path, @EditSessionID) AS Cat
        INNER JOIN [ReportingService_test2DBTempDB].dbo.ExecutionCache AS EC ON Cat.ItemID = EC.ReportID
        INNER JOIN [ReportingService_test2DBTempDB].dbo.SnapshotData AS SN ON EC.SnapshotDataID = SN.SnapshotDataID AND EC.ParamsHash = SN.ParamsHash
        LEFT OUTER JOIN SecData ON Cat.PolicyID = SecData.PolicyID AND SecData.AuthType = @AuthType
        LEFT OUTER JOIN Catalog AS Cat2 on Cat.LinkSourceID = Cat2.ItemID
    WHERE
